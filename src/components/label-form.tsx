...
export default function LabelCreationForm({ wallet }: LabelCreationFormProps) {
  ...
  const [members, setMembers] = useState<Member[]>([
    { address: "", isValid: true, name: "", pda: "", error: "" },
  ]);
  ...
  const validateMember = async (address: string, index: number) => {
    if (
      members.some((member, i) => member.address === address && i !== index)
    ) {
      return {
        isValid: false,
        name: "",
    ...
    try {
      if (wallet) {
        const { userAccount, userPda } = await fetchUser(
          wallet,
          new PublicKey(address)
        );
        return {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const initialMembers = members.map((member) => member.address);
      const result = await postLabel(initialMembers);
      setFeedback({
      ...
      // setIsDone(true); // 削除
    } catch (error: unknown) { // 修正
      if (error instanceof Error) { // 修正
        setFeedback({ error: error.message }); // 修正
      } else {
        setFeedback({ error: "Unknown error" }); // 修正
      }
    } finally {
  ...
  return (
    <form
      ...
      className="container mx-auto py-10 px-4 space-y-10"
    >
      {isLoading ? (
        ...
        feedback.tx ||
        feedback.error ? ( // 追加
        <CompleteCard feedback={feedback} /> // 追加
      ) : (
        <Card className="w-full max-w-2xl mx-auto">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {members.map((member, index) => (
                <div key={`member-${index}`} className="space-y-2">
                  ...
                  {member.error && (
                    <p className="text-red-500">{member.error}</p>
                  )}
            ...
            {feedback.squadId && <p>Squad ID: {feedback.squadId}</p>}
            {feedback.treeId && <p>Tree ID: {feedback.treeId}</p>}
            {feedback.tx && <p>Transaction: {feedback.tx}</p>}
            {feedback.error && (
              <p className="text-red-500">Error: {feedback.error}</p>
            )}
          </CardContent>
        </Card>
      )}
...
function CompleteCard({ feedback }: CompleteCardProps) {
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
